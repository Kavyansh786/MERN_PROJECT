...
export default function Earrings() {
  ...
  // Earrings sub-categories
  const categories = [
    { id: 'all', name: 'All Earrings', icon: '💎' },
    { id: 'studs', name: 'Studs', icon: '✨' },
    { id: 'drops', name: 'Drop Earrings', icon: '💫' },
    { id: 'hoops', name: 'Hoops', icon: '🔶' },
    { id: 'chandelier', name: 'Chandelier', icon: '💎' },
    { id: 'jhumka', name: 'Jhumka', icon: '🎵' },
    { id: 'korean', name: 'Korean Style', icon: '🌸' }
  ];
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        setFilteredProducts(earringsProducts);
        setError(null);
      } catch (err) {
        console.error('Failed to fetch earrings:', err);
        setError('Failed to load earrings collection.');
        showToast({
          type: 'error',
          message: 'Failed to load earrings collection.'
        });
      } finally {
        setLoading(false);
      }
      ...
      <div className="min-h-screen bg-gradient-to-br from-rose-50 to-pink-100">
        <div className="container mx-auto px-4 py-8">
          <div className="text-center">
            ...
            <p className="mt-4 text-rose-800 text-lg">Loading Earrings Collection...</p>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-[#fdf8f6]">
      ...
      <section className="max-w-7xl mx-auto px-4 md:px-8 pt-10 pb-2">
        <h1 className="text-4xl md:text-5xl font-extrabold font-serif text-[#3e2d26] mb-2 text-left">Earrings Collection</h1>
        <p className="text-lg md:text-xl text-[#8D6E63] mb-6 text-left max-w-2xl">Discover our stunning collection of earrings — from elegant studs to statement chandeliers. Perfect for every occasion and style.</p>
      </section>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8 justify-items-center">
          {filteredProducts.length === 0 ? (
            <div className="col-span-full text-center py-12">
              ...
              <p className="text-gray-500 text-lg mb-2">No earrings available yet</p>
