...
export default function Necklaces() {
  ...
  // Necklaces sub-categories
  const categories = [
    { id: 'all', name: 'All Necklaces', icon: '📿' },
    { id: 'pendants', name: 'Pendants', icon: '💎' },
    { id: 'chains', name: 'Chains', icon: '⛓️' },
    { id: 'chokers', name: 'Chokers', icon: '👑' },
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        setFilteredProducts(necklacesProducts);
        setError(null);
      } catch (err) {
        console.error('Failed to fetch necklaces:', err);
        setError('Failed to load necklaces collection.');
        showToast({
          type: 'error',
          message: 'Failed to load necklaces collection.'
        });
      } finally {
        setLoading(false);
      }
      ...
      <div className="min-h-screen bg-gradient-to-br from-rose-50 to-pink-100">
        <div className="container mx-auto px-4 py-8">
          <div className="text-center">
            ...
            <p className="mt-4 text-rose-800 text-lg">Loading Necklaces Collection...</p>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-[#fdf8f6]">
      ...
      <section className="max-w-7xl mx-auto px-4 md:px-8 pt-10 pb-2">
        <h1 className="text-4xl md:text-5xl font-extrabold font-serif text-[#3e2d26] mb-2 text-left">Necklaces Collection</h1>
        <p className="text-lg md:text-xl text-[#8D6E63] mb-6 text-left max-w-2xl">Explore our exquisite collection of necklaces — from delicate pendants to statement chokers. Elevate your style with timeless elegance.</p>
      </section>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8 justify-items-center">
          {filteredProducts.length === 0 ? (
            <div className="col-span-full text-center py-12">
              ...
              <p className="text-gray-500 text-lg mb-2">No necklaces available yet</p>
